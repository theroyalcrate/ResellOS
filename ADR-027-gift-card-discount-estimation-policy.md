...
Multi-retailer basket orders (LEGO bundled with other retailers' cards under one order-level discount): **correction, 2026-09-05 — no apportionment needed at all.** Kroger's own promo text itemizes each card's discount separately even within one bundled order (e.g. order 14590439: "Save 20% off a $75 Barnes eGift Card | Get a $25 bonus when you buy a $200 Lego eGift Card") — so each card in a basket gets its own real rate straight from the text, `exact`, no blending or pro-rata guessing. Fall back to applying the order's blended rate to every card only on the rare basket where the promo text doesn't break out per-item (`estimated_default` in that case).

**Other-retailer rates confirmed from the same 65-email pull (no new work — already-parsed data), for reference when those historical orders get entered:** Barnes & Noble 15% or 20% (two different promos seen), Kohl's 15%, Lowe's 10%, Bath & Body Works 20%, One4all (all sub-brands) 13.04%, Target 0% (bought at face value, no promo). Full detail in `/tmp/kroger_gift_card_orders.csv`.
