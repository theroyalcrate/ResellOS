...
20. **claude.ai Project custom instructions are stale (found 2026-09-06):** The Project's "custom instructions" field (Project Settings — a different setting from its doc list) still contains the full 2026-06-04 CONTEXT.md snapshot verbatim: old build state, "Current Session S10," and the pre-ADR-018 Kohl's-Cash-is-post-tax-tender framing. SESSION_LOG.md's Open Questions section had marked this "✅ RESOLVED 2026-06-21," but that only covered deleting the pasted knowledge-file copy — the separate instructions field was never cleared. No tool available to Claude can edit that field; **Josh needs to clear/replace it in claude.ai Project Settings directly.** Until then, any session on this Project without folder/GitHub access to the live repo starts from 3-month-stale context with no way to detect it.
---
