...
**Last Updated: 2026-09-04** — Walmart Business now has a real ingestion path: `walmart_business_import.py` (repo root) parses the official Order Data CSV export from business.walmart.com and writes orders/line_items to Supabase, idempotently. This replaces Walmart Business's slot in the Chrome Extension retailer-scope plan below (4 retailers were queued for content scripts as of 2026-08-22; now 3: Kohl's, Walmart, Macy's). First real run 2026-09-04: 26 orders, $7,167.34, 2026-07-20→09-01. See SESSION_LOG.md "Cowork 2026-09-04" entry for full detail, including three real export-format quirks the parser has to handle correctly. Prior update: 2026-07-31 — ResellOS Chrome Extension recon thread under Planned Future Systems completed for all five originally scoped retailers (LEGO.com, Kohl's, Walmart, Walmart Business, Macy's). Walmart Business's 2% rewards open question (Open Questions section) partially confirmed with a newly-found $1,000/year cap. Build State table was also refreshed 2026-07-30 to reflect Agent 09/10 and the 2026-06-20→07-19 Cowork sessions.
**Read this first. This document orients Claude at the start of every conversation.**

> **Document home & sync rule (revised 2026-06-21):** This repo is the only copy. The previous "identical copy pasted into the Claude project" was deleted on 2026-06-21 because it kept drifting out of date and caused sessions to start from stale context (this exact conversation started from a 2026-06-04 snapshot while this file had moved on to 2026-06-18). Every Claude surface — plain chat, Cowork, or Claude Code — now reads this file (and SESSION_LOG.md, and CLAUDE.md) live from the repo, or fetches it fresh from GitHub if it has no folder access. Never rely on memory or a cached copy. Enforced by the `resell-os-session-start` skill (`skills/resell-os-session-start.md`) — see CONTEXT.md → "How to Use This Document" below. **Separate gap found 2026-09-06:** the claude.ai Project's own "custom instructions" field (Project Settings, not the doc list) still held the stale 2026-06-04 snapshot this paragraph describes killing — see Open Question #19.
...
| Pre-S10 Agent 1B+1C | Agent 1B extended: Mode 4 personal Gmail backfill (copy personal LEGO emails → business Gmail, idempotent), Mode 5 personal safety-net filter (labels e.lego.com emails ResellOS-Needs-Copy). Agent 1C scope folded in — no separate Agent 1C session needed. 4 code-review bugs fixed. Mode 2 live-tested 2026-07-18; Modes 4/5 still untested. | ✓ Mode 2 tested |
| Cowork 2026-06-20 → 2026-07-14 | Auth-vs-enrichment scraping architecture decided; ADR-020 (cost basis regression testing) proposed; ADR-021 (FIFO locked), ADR-022 (cashback Layer 4 + Capital One chain) written; non-LEGO gift card import (175 rows); `email_enricher.py` LEGO parser built (908 lines, 72 tests); pipeline audit + retailer casing fix; Zapier Gmail/Drive connector verified (personal account, separate from Agent 1B/1C's business-account OAuth). Full detail: SESSION_LOG.md Session History. | ✓ Done |
| Cowork 2026-07-18 | OAuth publish-status fix (resolves 7-day token expiry); `invoice_files` schema drift found + fixed live; **first successful Agent 1B live filing** (order T469280178, verified independently); Tier 2 PDF-content matching gap found (LEGO "Receipt" emails have no order number in the subject — blocks bulk-filing the ~200-email backlog); manual-entry-first order architecture decided (email agents verify/enrich, never originate orders); Agent 09 Purchase Planner built. | ✓ Done |
| Cowork 2026-07-19 | Agent 10 buy-side stock/discount watch built — migration 016, 19 High-tier sets seeded, only Walmart checker proven live, 23 tests. Sell-side price-watch tool scoped but paused (Josh wants to think it through more). | ✓ Done |
| S10 | Phase 3: variable-earn schema (per-order observed rewards + Kohl's Cash block model with explicit expiration) + earn cliff pin against June 8th orders, competing for priority against Tier 2 PDF-content matching for Agent 1B (see SESSION_LOG.md "Start Here — Next Session" for the live call on order). | → Next |
...
| Walmart Business | 2% on orders over $250 — calculated on original order value at placement, NOT final total | Pickup, delivery, AND in-store ("Wmt Store (non-eComm)") purchases all occur — confirmed 2026-09-04 from a real order-data export. Orders now come in via `walmart_business_import.py` (CSV export from business.walmart.com), not manual Agent 02 entry or a browser extension. |
...
2% likely calculated on original order value at placement, not final pickup/invoiced total. One real order: $290 placed, $204 final pickup, $8.40 rewards credited. **Partially confirmed 2026-07-31** via live recon of `business.walmart.com`'s Rewards Center: the final charged total is cleanly ruled out as the basis (2% of it doesn't come close to the reward shown), and the placed/hold value is much closer but still not an exact match — some items/categories may be excluded from eligibility. Also newly found: **the 2% program has an explicit $1,000/year cap**, not previously documented. A second live attempt the same day (order #200015136689078, "9078") also failed to reconcile cleanly — the order was still mid-fulfillment (not yet picked up, Charge History showing a $822.79 hold and two adjustments but no final charge yet), so none of the candidate bases matched the pending $12.72 "2% rewards" figure. **Recheck this exact order after Josh picks it up ~2026-08-01, once the final charge and final reward amount post** — that's the number to reconcile against, not an in-flight one. Full detail in Cowork session memory (search "Walmart extension recon findings").
...
**ResellOS Chrome Extension (Recon phase started 2026-07-30/31; LEGO.com v1 built 2026-08-21 — see SESSION_LOG.md, NOT YET TESTED end-to-end)**
The long-term replacement for both manual order entry and invoice/email parsing: a content script that captures order data live, at the source, on the retailer's own order-detail page — instead of reverse-engineering a printed receipt or a confirmation email after the fact. First raised as the intended end-state in the manual-entry-first decision below; became a live thread on 2026-07-30 when Josh confirmed browsing (Claude in Chrome) is good enough now to do real reconnaissance on retailer pages rather than guessing at DOM structure secondhand.

*Retailer scope, per Josh (2026-07-30):* Walmart, Walmart Business, Macy's, Kohl's, and LEGO.com. The first four are the harder case for a parser — their printed receipts/invoices don't carry enough usable data (consistent with Kohl's rewards already needing to be read off the invoice rather than computed, and Walmart's 2% basis being ambiguous from the invoice alone). LEGO.com's receipts already parse fine via Agent 1A, but Josh wants it in the extension too anyway, for build simplicity — one shared extension covering the highest-volume retailer rather than a data-availability need. Target and Best Buy weren't mentioned — not yet clear if that's deliberate.

**Update 2026-09-04 — Walmart Business dropped from this list.** Josh found business.walmart.com's own "Order Data" export (Reports → Order History) covers the same ground as a content script would, with no browser automation needed — full line-item detail, cancellations, and per-item tender type, on demand. `walmart_business_import.py` (repo root) parses it. Scope for the Chrome extension's remaining build is now Kohl's, Walmart, and Macy's only. See SESSION_LOG.md "Cowork 2026-09-04" for the build and first real run.

*LEGO.com recon (done):* Order list at `/en-us/member/orders` and order detail at `/en-us/member/orders/details/{orderNumber}` (real anchors, not JS-only clicks). Detail page is client-rendered — needs a wait-for-DOM approach, not read-on-load. It exposes, cleanly: full delivery/billing address, **payment method with card last 4 digits** (closes the historical gift-card-linkage gap noted below — no LEGO receipt has ever printed which card was used), order summary (subtotal/tax/discount/total), tracking info, and line items with exact set number ("Item: {number}"), quantity, and price — shown as strikethrough MSRP + Insiders price when discounted, or a bare dash "–" with no price for $0 items, a strong GWP-detection signal matching ResellOS's existing $0-GWP convention. Points History (`/insiders/account#earn`) is a real per-transaction ledger (date + "LEGO Purchase" + points delta), though it wasn't confirmed whether a row links back to a specific order number.

*Kohl's recon (done):* Order list/detail at `/myaccount/purchases` (detail opens as a slide-out panel but the URL still updates to `?orderId={orderId}`, so it's matchable). The order summary shows **Kohl's Cash redeemed as its own line item** (e.g. -$185.00) and **Kohl's Rewards Earned for that order**, both printed directly — no computation needed. Separately, **Rewards Activity** (`/myaccount/rewards-activities`) is a full ledger where each earn row expands to show the **literal earn rate** (e.g. "5%"), transaction amount, dollars earned, and a "View Order Details" button linking straight back to the order — directly answering, for Kohl's, the order-linkage question left open by the LEGO recon. A monthly "Kohl's Cash — Rewards Issuance" ledger entry converts accumulated Rewards into spendable Kohl's Cash (e.g. $185.00 issued June 30, matching the $185.00 redeemed on a July 1 order) — confirms the "$5 increments, valid 30 days" model, though an expiration date / certificate identifier per instance wasn't found on that ledger row; still need to check the dashboard's Kohl's Cash widget for that. **This likely makes S10's "read rewards off the invoice" plan unnecessary for orders placed after the extension exists** — invoice-reading would still be needed for historical backfill.

*Walmart (consumer) recon (done):* Order detail at `/orders/{orderId}?groupId=...` (real navigated page). Line items have no separate labeled set-number field (unlike LEGO.com) — set number sits inside the free-text title only, consistent with the existing architecture note that Walmart→inventory matching relies on UPC/EAN via Brickset, not a printed set number. **Biggest find: the "Charge history" panel exposes the full temp-hold → order-adjustment → final-charge lifecycle** (e.g. $225.00 hold → -$140.00 adjustment for substitutions/OOS → $85.00 final charge). This is the literal mechanism behind the already-documented "$290 placed, $204 final" example — not a rewards mystery, just Walmart's standard substitution flow. Implication for the extension: reading only the settled order page will under-report the original cart value; anything needing the placed value must catch the order at placement or read Charge History specifically. Walmart Cash (`/walmartcash`, `/cash-overview`) is confirmed **100% manufacturer-offer based** (fixed $ per specific product, not a % of order) — a different mechanic from Walmart Business's 2% program below, despite shared branding.

*Walmart Business recon (done):* Separate site (`business.walmart.com`), same login via SSO. Genuinely distinct account model — Organization info, Users & groups, Tax exemption, a PO number/reference ID field per order, Spend Analytics, Reconciliation, Approvals, and a payment method can carry a **"Shared"** tag (pooled org card). The **Rewards Center** (`/account/rewards-center`) resolves the CONTEXT.md open question on the 2% program: it's real, and there's an **explicit $1,000/year cap** on "Annual 2% rewards" — not previously documented anywhere in ResellOS. Each reward ledger row is tagged "2% rewards" or "Manufacturer offers" and links directly to its order. Checked one order in full: $689.15 placed hold → two substitution adjustments → $133.08 final charge, with a $10.00 "2% rewards" line for that order. 2% of the $133.08 final total ($2.66) is cleanly ruled out as the basis; 2% of the $689.15 hold ($13.78) is closer but still not an exact match — **the precise eligible-spend definition (some items/categories may not qualify) is narrowed but not fully pinned down**, needs more order data points.

*Macy's recon (done, but thin — no settled order available to check against):* Order detail at `/purchases/online/{orderNumber}`. Same free-text-embedded-set-number pattern as Walmart, plus a "View all transactions" panel that splits the charge by tender (Star Money vs. card) — same principle as Kohl's Cash/LEGO's gift-card-last-4, just not observable with real numbers this session since Josh's only recent order hadn't shipped/charged yet. **Star Rewards** (`/loyalty/starrewards`) confirms a clean, deterministic ratio — **1,000 points = $10 Star Money**, 1 point per $1 spent — notably simpler than Kohl's variable rate. Star Money expires 30 days after issuance (same window as Kohl's Cash — a real cross-retailer pattern). The "How it works" copy's "with your Macy's Card" wording looked like it might restrict earning to the Macy's-branded card — **Josh confirmed 2026-07-31 from direct experience this is not the case: points accrue on any tender, the only real exclusion is that the gift-card-paid portion of a purchase doesn't earn points.** Distinct from the existing open item #6 (Star Money pre-tax vs. post-tax, still needs a real invoice).

*Status:* Recon complete on all five retailers as of 2026-07-31 (Walmart/Walmart Business/Macy's thinner than LEGO.com/Kohl's — especially Macy's, which had no settled order to check rewards against). Full findings and exact URLs live in Cowork session memory, richer than what's condensed here — redo/confirm each walkthrough live before writing content-script code rather than trusting this summary as a spec. Josh wants this built in Claude Code now that all five are scouted — see SESSION_LOG.md "Start Here — Next Session" for current sequencing against S10/Tier 2 matching.

**LEGO.com v1 built 2026-08-21 (`extension/`, NOT YET TESTED end-to-end — see SESSION_LOG.md).** Live DOM verification that session corrected one thing this recon summary got ambiguous: LEGO's per-line price display (both the single price and the strikethrough-MSRP/sale-price pair) is a **line total for the full quantity, not a per-unit price** — e.g. a qty-4 line showed "$131.96" / "$105.56", and only dividing by 4 ($32.99 / $26.39) is consistent with the order's real Subtotal. Anything that reads a LEGO order-detail page's line prices (a future content script, a manual QA check, Agent 1D if it ever needs this page) must divide by quantity to get true per-unit pricing before it hits code that assumes per-unit (as ADR-023's `raw_data` contract and `capture_queue_promotion.py` both do). Also confirmed live: "Item: {number}" renders as two separate sibling elements, not one text node, and LEGO's item-row container sits under a per-build hashed CSS-module class name that will change on redeploy — don't hardcode it, key off nearby text ("Quantity") instead.
...
**Purchase Planner Agent (Phase 2 — Agent 09 — A-002, designed 2026-07-18)**
Standalone script, separate from agent_02: `agent_09_purchase_planner.py`. One purchase plan per planned buying session, not per order — created ahead of time when Josh knows a sale window, double-points event, or GWP threshold is coming up. Per plan he enters: target sets (price, and whether each is GWP-eligible or part of a sale/promo), which gift card(s) he intends to use and their current balance, available Insider points, and a target — a GWP spend threshold, a points/tier threshold, or a plain spend cap. GWP and sale thresholds are always manually entered, never predicted — real patterns exist but change often enough that automating the guess would just introduce noise (decided 2026-07-18). The calculator finds the combination of sets/quantities that clears the threshold with the least overspend, and shows the resulting gift card balance and points earned — showing its work (which combo, why) rather than a black-box number, since Josh needs to sanity-check it before buying. Plan status: draft → ready → placed, the same pattern already used for hit list (active/purchased/abandoned). When the order is actually placed, Josh enters the real order number and the plan's line items pre-fill agent_02's entry screen — he confirms against the real invoice instead of retyping, which is what closes the "no double entry" requirement. Still consumes hit list as its input queue. Output feeds directly into Agent 02 order entry.

**Buy-Side Stock & Discount Watch (Agent 10 — built 2026-07-19, partially verified)**
`agents/agent_10_stock_watch.py`. Watches for restocks and MSRP-relative discounts across the 7 retailers where daily cadence is acceptable: Macy's, Target, Kohl's, Walmart, JCPenney, Christianbook, Amazon. LEGO.com seconds-level restock alerts are explicitly out of scope — Josh already pays for a dedicated service, and general-purpose scraping actors aren't built for that cadence economically (per-run browser/proxy overhead vs. a dedicated service's shared infrastructure across many subscribers). One watch target per LEGO set (not per set+retailer) in `stock_watch_targets` — which retailers carry a given set isn't known ahead of time, so a daily check fans out across all active retailers per set and discovers carriers naturally; a retailer not carrying a set is expected, not an error. Alerts fire on a restock (out-of-stock → in-stock) or a discount ≥ threshold % below MSRP (default 20%), both independently, either or both can fire on the same check. Seeded 2026-07-19 with 19 High-tier sets from Brick Domain's July 2026 tier list (Josh's uploaded retiring-sets source — reliable per prior project notes, vs. Brickeconomy which is flagged unreliable for retirement prediction). **Only the Walmart checker is proven** (confirmed live 3x via its own Apify actor with real price/stock fields) — Target, Kohl's, Macy's, JCPenney, Amazon, and Christianbook all need real verification before they're trusted; wrong data in an alert tool is worse than no data, so unverified checkers were deliberately left unwired rather than shipped with a guessed field mapping. Runs standalone (calls Apify + Firecrawl REST APIs directly, not through Cowork's MCP connectors) — needs `APIFY_API_TOKEN` and `FIRECRAWL_API_KEY` added to `.env` before it can execute outside a Cowork session. Full verification detail and per-retailer status: see the module docstring in `agent_10_stock_watch.py` and the 2026-07-19 SESSION_LOG entry.

**Sell-Side Price Watch (scoped 2026-07-19, not yet built)**
Different tool, opposite direction from Agent 10 — watches Amazon, Walmart, Barnes & Noble, Hobby Town, and Macy's for competitor stock still selling at MSRP, which suppresses secondary-market appreciation on sets Josh already holds inventory of (example given: 76178 Daily Bugle had "zombie stock" at Amazon most of the year, went OOS the week of 2026-07-19). Business context: Josh is shipping most of last year's purchased inventory into WFS starting mid-August 2026, targeting fully in WFS by beginning of October for Q4, and wants pricing that preserves ROI on those sets. Josh wants to think through the design more before building — not a build task yet.
...
| Gift card bulk-import & review queue | Bulk-uploaded gift card ledgers (last4 + face value only, no purchase economics) never write straight into `gift_cards` — its `purchase_price`/`purchase_date` are `NOT NULL` by design (ADR-027). Instead they stage in `gift_card_import_queue` (migration 021) via `agents/agent_11_gift_card_bulk_import.py`, and only reach `gift_cards` (as `discount_confidence='exact'`) once Josh confirms purchase date/price/source per row (or per batch) in `gift_card_import_review_app.py` (Streamlit). Mirrors the capture_queue → review → promotion shape (ADR-023/028). Order↔gift-card linkage is explicitly out of scope of this decision. (ADR-030, DECISION 2026-09-14) |
| Gift card balance tracking & order confirm/reopen lifecycle | The gift card ledger atomic write DECISION 017 specified 2026-06-01 (debit at confirm, reverse on reopen to pending_review) is now built, not just decided — `gift_card_ledger.py` + `order_lifecycle.py`. Debit fires when an order is written or transitioned to `order_status = confirmed`; reopening a confirmed order reverses the linked `gift_card_assignments` row(s) and credits the balance back; cost basis Layer 2 reads the ledger (amount × card `discount_pct`) instead of a manual re-entry. Forward-only — no 2025 retroactive rebuild. One card per order assumed for v1. (ADR-031, DECISION 2026-09-15) |
...
| Manual-entry-first order architecture (Decided 2026-07-18, SUPERSEDED 2026-08-18 by ADR-023) | Originally: Josh enters every order himself via `agent_02` until the extension exists, because email/receipt data was unreliable. Superseded once Tier 2 PDF matching (2026-08-18) proved extraction reliable — see ADR-023 for the replacement architecture (extension primary, PDF/email agents as backstop, both feeding a shared `capture_queue` review gate). Agent 02 manual entry remains the fallback for retailers the extension doesn't cover. |
| Purchase Planner design (Agent 09) | Standalone script, `agent_09_purchase_planner.py` — deliberately separate from `agent_02`, not a mode within it. One purchase plan per planned buying session (not per order): target sets + prices + GWP/sale eligibility, gift card(s) to use + current balance, available Insider points, and a target (GWP spend threshold, points/tier threshold, or plain spend cap). GWP/sale thresholds are always manually entered, never predicted — real patterns exist but change too often for automated prediction to be worth the complexity. Calculator finds the minimum-overspend combination that clears the threshold and shows its work (not a black box), plus resulting gift card balance and points earned. Plan status draft → ready → placed, mirroring hit list's active/purchased/abandoned. Placing the real order pre-fills `agent_02` from the plan's line items — Josh confirms against the invoice rather than retyping, closing the original "no double entry" requirement from A-002. Consumes hit list as its input queue. (Decided 2026-07-18) |
| Buy-side stock-alert scope: LEGO.com excluded, 7 retailers at daily cadence (Decided 2026-07-19) | Josh already pays for a dedicated LEGO.com stock-alert service that does seconds-level restock notification — general-purpose scraping actors (Apify/Firecrawl) spin up a browser/proxy session per check, so seconds-level polling isn't cost-competitive with a service that shares infrastructure across many subscribers watching the same pages. LEGO.com restock alerts stay out of scope for Agent 10 unless Josh provides his actual subscription cost for a real comparison. Agent 10 covers Macy's, Target, Kohl's, Walmart, JCPenney, Christianbook, Amazon at daily cadence instead, where the economics clearly work. |
...
2. **Google Drive migration** — UPDATED 2026-08-15: scale confirmed much larger than assumed. A Zapier search of the personal Drive for LEGO receipt/invoice PDFs (`name contains 'LEGO' or name contains 'Receipt'`) hit the 1,000-result page cap on the first page alone, with more beyond that (`nextPageToken` unused) — 714 distinct filenames within that page, 228 of which have duplicates (286 extra copies). Still fragmented across multiple named folders as described below, not one place: "Lego inventory and purchase tracker." (LEGO-only, consistently month-organized), "The Royal Crate LLC receipts" (an abandoned consolidation attempt, started and last touched ~April 2025), plus separate ad-hoc root-level folders per retailer created after that attempt stalled ("Walmart Business Purchases," "Walmart.com," "Walmart Missed?," "Target receipts," "Barnes And Noble," "Kohls," "Kohl's 2026," "Best Buy"). Full detail: `references/retailer_email_sources.md`. **Plan (decided 2026-08-15, scheduled for 2026-08-16):** write a script extending Agent 1C's historical-backfill pattern but sourced from Drive file listings instead of Gmail messages — dedupe by invoice number, copy survivors into the business `Invoices/Lego/<year>/<month>/` structure. A complete root-folder inventory (file counts, date ranges per folder) is still not built and should be the script's first step.
...
13. ~~**Duplicate line items**~~ ✅ CHECKED CLEAN 2026-08-18 — cross-path (manual + parser) historical artifact, originally flagged 2026-07-18. Verified directly against live Supabase 2026-08-18: zero duplicate line items across all orders (grouped by order + set_number + set_name + quantity + unit_price, no group has more than one row). Not currently blocking anything. `order_validators.py`'s cross-shipment collision check (added 2026-06-21, see the Duplicate line items architecture note above) guards against new duplicates going forward, but worth a repeat check before Agent 1D (ADR-023) goes live, since that introduces a new write path (PDF backfill → capture_queue → promoted order) that hasn't been exercised against this check yet.
...
17. **Gift card savings (cost basis Layer 2) is never persisted (new 2026-06-21)** — `agent_08_cost_basis.py` Mode 1 collects it interactively and uses it in that one calculation, but writes it nowhere. No way to independently audit or re-derive a past `net_economic_cost` later — only the final per-unit `inventory.cost_basis` survives. Would need a schema addition before any cost-basis audit feature is possible. **See #21 below — this turned out to be a symptom of a bigger, unwired gap, not just a missing persistence step.**
...
19. **Amazon Business delayed shipment reward (new 2026-06-07, carried over from SESSION_LOG.md's now-retired Open Questions section, 2026-09-06):** 1% credit offered occasionally at checkout for accepting 1–3 day delivery delay. Offered inconsistently — must be prompted at order entry. Capture in `rewards_transactions`. Redemption path on future invoices not yet documented.

20. ~~**claude.ai Project custom instructions are stale (found 2026-09-06)**~~ ✅ RESOLVED — the Project's custom-instructions field now holds the deliberately-static evergreen text described in the audit's own recommendation (confirmed live 2026-09-14: it explicitly tells the reader not to trust it for current state and points at the repo). The Project's doc list was also cleaned up per the same audit — the 6 stale/duplicate docs it flagged (2× Master Architecture Document, 2× Project Map, `reselling-os-product-doc.html`, the Ideas doc) are gone, leaving `barnes.md`/`kohls.md` (current, per the audit) plus the audit itself and the new evergreen-instructions doc. Nothing unique was lost — every removed doc's real content already lives current in this repo (CONTEXT.md/SESSION_LOG.md/ADRs); they were confirmed-stale duplicates, not sources. One pre-existing, not cleanup-caused gap remains: `macys.md` (built 2026-06-07 per item 6 above) still isn't in this Project's doc list — presumably still only in the ResellOS-Knowledge vault repo. Worth adding if Josh wants it there for convenience.

21. ~~**Gift card balance tracking is structurally unwired, not just imprecise (found 2026-09-14, Josh flagged directly)**~~ ✅ RESOLVED 2026-09-15 — see ADR-031 (gift card balance tracking, wired to the order confirm/reopen lifecycle). Original text preserved below for context. — `gift_card_assignments` exists as a table with the right shape (`card_id`, `order_id`, `amount_applied`, `applied_date`) but has only **18 rows** against **366** `gift_cards` rows — essentially unused. No write path populates it: `agent_02_order_entry.py`, `capture_queue_promotion.py`, and the Chrome extension all capture `gift_card_applied` as a free-floating dollar figure on the order, never tied to a specific `card_id` (explicitly flagged as a known gap in `capture_queue_promotion.py`'s own docstring: "no gift_card_assignments... writes exist in that path"). `gift_cards.remaining_balance` is set once — at manual entry (= face_value) or at bulk-ledger-import time (sometimes reflecting a historical partial-use snapshot from the source spreadsheet) — and is never decremented afterward by any code path found; of 366 cards, 159 show a `remaining_balance` different from `face_value`, but that's import-time snapshot data, not evidence of live debiting. Cost basis Layer 2 (`agent_08_cost_basis.py`) doesn't read the gift card ledger at all — every order re-prompts Josh to manually type a "Gift card savings ($)" figure, completely disconnected from any specific card record (this is what item 17 above was really pointing at). **Josh's ask (2026-09-14): not interested in retrofitting 2025 usage on the messy legacy sheet, but wants currently-active balances and all newly-purchased cards to track correctly going forward, feeding correctly into cost basis.** Real, scoped next-session candidate — the schema piece already exists and is well-formed; this is a write-path/read-path wiring problem: (1) order entry should let Josh pick a specific `gift_cards` row and write a real `gift_card_assignments` row, (2) that write should debit `gift_cards.remaining_balance`, (3) cost basis Layer 2 should read `gift_card_assignments` for the order instead of asking again. Worth an ADR before building — touches order entry, capture_queue_promotion, the cost basis engine, and possibly the gift-card review app from tonight's ADR-030 work (a natural place to also set each card's *initial* remaining_balance correctly for currently-active cards).

22. **`agent_1e_pdf_backfill` can silently drop an entire shipment on a multi-box order — no existing validator catches it (found 2026-09-17, Cowork session).** For orders that ship in multiple separate boxes, `agents/agent_01e_pdf_order_backfill.py` sometimes only ever finds/parses ONE of the invoice PDFs, leaving the order missing a whole shipment's line items. The order still looks internally "reconciled" (its `subtotal` matches its own incomplete line items), so this passes every check in `order_validators.py` undetected — it is a genuinely new class of data-quality gap, not a variant of anything already checked for. Found when Josh recaptured order T450671168 via the Chrome extension and noticed a missing line item. **Detection technique (documented in full at `references/agent_1e_missing_shipment_detection.md`):** for each `entry_method='agent_1e_pdf_backfill'` order, count LEGO's own "Your LEGO Order T###### is on its way" shipping-confirmation emails (sender `Noreply@t.crm.lego.com`) per order number via Gmail search, and compare against the count of `shipments` rows in Supabase for that order_number — where the email count is higher, that's a high-confidence candidate, confirmed by reading the actual email bodies (which carry per-shipment subtotal/tax/total and item list). Run once against all 592 `agent_1e_pdf_backfill` orders: 8 candidates surfaced, **6 were real gaps and fixed via direct SQL** (T450671168, T509174028, T507663840, T509176263, T508221251, and T512438222 — the last of which didn't exist in the database at all and was built from scratch from its Gmail emails), 2 were false positives (a $0 promo item shipped separately; a manual-entry order missing only cosmetic shipment/tracking rows, not line items), and 1 further candidate (T508133224) was confirmed by Josh to be accurate as-is — LEGO simply never sent a complete notification email for that order. Full order-by-order detail in SESSION_LOG.md's 2026-09-17 Session History entry. **Also surfaced in the same investigation:** LEGO's own order-details "shipped" page (what the Chrome extension's `content.js` scrapes) shows inaccurate historical pricing/quantity data on old orders — it mislabels allocated tax as a price "discount" and can show wildly wrong shipped-quantity totals (one order showed "6 shipped" against 3 units ever ordered/received). Don't trust a Chrome-extension recapture of an old order's pricing or quantity fields against that page; Gmail shipping-confirmation emails are the reliable source for reconstructing historical shipments. **Not yet built (as of 2026-09-17):** no code change was made to `agent_1e_pdf_backfill`, `capture_queue_promotion.py`, or `order_validators.py` this session — this was a data-correction pass only. The email-count-vs-shipment-count check was run once, by hand, against one `entry_method` (592 orders); it hasn't been automated, added as a standing validator, or extended to other entry methods (manual entry, `capture_queue_promotion.py`, the Chrome extension, `walmart_business_csv_import`) where the same class of gap could in principle also exist.

**UPDATE 2026-09-18 — the underlying architecture bug is now fixed in code** (not yet committed, not yet run against production — see SESSION_LOG.md's 2026-09-18 Session History entry for full detail). Root cause confirmed: `agent_1e_pdf_backfill` used to write orders directly instead of going through `capture_queue` (ADR-023's "single review gate"), and never compared newly-found PDF data against an order that already existed. Both are fixed: the agent now always writes into `capture_queue` first; a new `order_validators.find_missing_line_items()` compares newly-found data (from a PDF or a later Chrome-extension capture) against what's actually on file for an order, and flags — never silently auto-fixes — a real mismatch. The same fix was applied to `capture_queue_promotion.py`'s `_merge_shipped_capture()` (had the identical gap) and to `capture_queue_review_app.py`'s Streamlit review path (initially missed in code review, since the CLI got the fix first — same class of gap this whole issue is about, recurring one level up). A non-interactive mode was added throughout so this can run unattended on a schedule. 245 new/updated tests; extensive code review (multiple rounds) caught and fixed 15+ real bugs along the way, including one that would have violated the cost-basis-locking rule below (a new "add missing items" function could have silently rewritten a `confirmed` order's totals — now refuses outright) and one that risked writing duplicate line items under a specific race between two review UIs (now re-verifies against the order's current state immediately before writing, rather than trusting a possibly-stale snapshot). A handful of lower-severity findings (DRY duplication between the CLI and Streamlit review paths, some DB round-trips that could be batched, `_map_line_items()`'s pre-existing MSRP/net-price imprecision affecting Chrome-extension orders too) were deliberately left as documented follow-ups rather than expanding this already-large session further — see the Session History entry for the full list. **Committed 2026-09-18 as `7f57b04`, pushed to `origin/main`.**

**UPDATE 2026-09-19 — the 18-file "_unmatched/" residue investigated and closed out; agent now finishes the job on a successful match.** A closer look at what was left in Agent 01D's backlog after the above fix found it wasn't a parser bug at all: 18 files were permanently non-LEGO.com-order documents (8 pure noise — Home Depot/donation receipts, LEGO Insiders points-history screenshots — and 10 real purchases already tracked separately in Brickprobe, in a retailer/format this LEGO-only PDF agent was never going to match: Walmart.com, Fred Meyer, an in-store purchase, gift-card purchases). New `invoice_files.excluded_reason` column (migration 022) marks a row as permanently dismissed from the backlog without touching the underlying Drive file — reversible by clearing the column. All 18 stamped (`not_an_order` ×8, `other_retailer_tracked_in_brickprobe` ×8, `gift_card_purchase` ×2). Separately, Agent 01E now also relocates the physical PDF out of `_unmatched/` into its real `{Retailer}/{Year}/{Month Year}/` folder and renames it to the standard convention on a clean write or confirmed match, reusing Agent 01B's own folder-resolution functions — closing the other half of the original gap (a matched order used to leave its source file behind in `_unmatched/` forever). Code review caught a real shipment-numbering collision risk in this feature before it ran live (see SESSION_LOG.md's 2026-09-19 entry). Backlog now genuinely empty other than 3 legitimate rows. Full detail in SESSION_LOG.md's 2026-09-19 Session History entry.

23. ~~**Agent 01B's live Gmail-filing pipeline needs a scheduling decision — same shape as the Agent 01E gap just fixed above: it works correctly when run, but nothing makes it run regularly (found 2026-09-19, Cowork session).**~~ ✅ RESOLVED 2026-09-19 (Claude Code, same night) — **decision: both Agent 01E and Agent 01B run on a weekly Windows Task Scheduler job, Saturday night.** Chosen deliberately so Sunday ResellOS sessions start with freshly filed, current data rather than a stale backlog, rather than daily (Agent 01E's own steps use a daily example, but weekly on the same night keeps both agents' schedules aligned and matches how often Josh actually works ResellOS). Agent 01B gained the same `--run`/`--preview`/`--report` non-interactive mode Agent 01E already had, so the interactive-only gap this question raised is closed — see SESSION_LOG.md's 2026-09-19 (second) Session History entry for the Task Scheduler setup steps and the full list of bugs found and fixed along the way (a real cross-run shipment-numbering collision, the same bug class as Open Question #22, independently reintroduced here and fixed the same way). **Important operational note, not just a scheduling detail: the FIRST Agent 01B `--run` (the ~1,169-message historical catch-up in `ResellOS-Invoices`, going back to February 2025) must be run manually and supervised once, before the recurring Saturday schedule is turned on for it** — see SESSION_LOG.md for why. Original text preserved below for context. — The business Gmail account's `ResellOS-Invoices` label currently holds **1,169 messages** (confirmed live, spanning 2025-02-23 through 2026-09-15 — as recent as 2 days before this was checked), all genuine LEGO receipt/order-confirmation emails, and the label is still actively receiving new mail today — the intake side is not broken. But the `invoice_files` ledger shows only **10 rows total** were ever created by the live Gmail-filing pipeline (as opposed to the separate Drive-backfill batch) across the pipeline's entire history, all in two short manual bursts (1 on 2026-07-19, 9 on 2026-08-18) — not a steady trickle, and not "around September 4th" as an earlier note assumed (direct query confirms August 18 as the actual last activity). In other words: Agent 01B's Gmail-intake mode has essentially never been run as an ongoing process — it's been manually invoked at most twice, while well over a thousand messages have piled up untouched since February 2025.

24. **Agent 01D's `count_shipments()`-based shipment numbering has the same cross-run collision bug as Open Question #22 and #23, not yet fixed (found 2026-09-19, code review during the Agent 01B --run build).** `agents/agent_01d_drive_historical_backfill.py` (line ~325) still computes `count_shipments(order["order_id"], client) + 1` directly — the same stale-count problem already found and fixed in Agent 01E (2026-09-17) and Agent 01B (2026-09-19, this session): filing/relocating an invoice never inserts a `shipments` row, so a split-shipment order backfilled across two separate Agent 01D runs can still be assigned the same shipment number twice, colliding on the same Drive filename. Not fixed this session — flagged during Agent 01B's code review but judged out of scope for a task specifically about Agent 01B's `--run` mode; Agent 01D's own correctness properties weren't otherwise audited. Worth its own dedicated pass, reusing the same `invoice_files.filed_filename`-counting pattern already proven twice (`_next_shipment_number()` in both `agent_01e_pdf_order_backfill.py` and `agent_01b_invoice_filing.py`).

25. **Shipment/invoice numbering has no single source of truth across the codebase — the `_next_shipment_number()` fix in #22/#23 traded one staleness bug for a narrower one, and multiple review passes (2026-09-19, building Agent 01B's `--run` mode) converged on the same underlying gap from different angles.** Three related, independently-confirmed issues, none fixed this session:
    - **`record_filing()` writes exactly one `invoice_files` row per Gmail message, even when `_execute_filing()` uploads N PDFs from one message as ship1..shipN.** Only the ship1 file/name is ever logged. Since `_next_shipment_number()` (both agents' copy) counts `invoice_files` rows to compute the next number, any past multi-PDF-attachment message permanently under-records — a later invoice for that same order will eventually recompute a shipment number that's already occupied in Drive by an unlogged ship2+ file, reopening the exact cross-run collision #22/#23 were meant to close. In-batch, this is masked by `mode_run_batch()`'s in-memory `filed_this_batch` tracker (keyed on the real `pdf_count`, not the ledger) — but that correct count is lost the moment the process exits, so it doesn't help the NEXT run. Root fix candidates: log one `invoice_files` row per uploaded file instead of per message, or move shipment-number computation to write time inside `_execute_filing()` itself (querying fresh, immediately before each `build_filename()` call) instead of once during `mode_preview()`'s read-only scan — the latter would also let `mode_run_batch()` drop its in-memory tracker entirely for the ordinary case, though it doesn't by itself fix the ledger under-recording.
    - **`_next_shipment_number()` (both agents' copies) is blind to the `shipments` table, which is a real, separate write path** — `db_writer.write_invoice()` (called from `invoice_parser.py <pdf> --db`, i.e. "Agent 1A," the original P0/S02/S04-era LEGO invoice tool) inserts directly into `shipments` for an order that Agent 02 already created, independent of whether that invoice ever passed through Gmail intake. **Currency of this path is unconfirmed** — no session log entry since the 2026-07-18 manual-entry-first / ADR-023 `capture_queue` architecture shift mentions it being run with `--db`, so it may well be legacy/dead in current practice, but that hasn't been verified either way. If it's still used even occasionally, an order shipment-numbered via that path and later invoiced through Agent 01B's Gmail pipeline (or vice versa) could collide, since the two numbering sources (`shipments` rows vs. `invoice_files.filed_filename` rows) don't see each other.
    - **Minor, lower-confidence, unverified:** `_next_shipment_number()`'s de-dupe key is a filename prefix built from the order's *current* `order_number`/`retailer` fields, while `invoice_files.filed_filename` is a frozen string written at file time — if either field is hand-corrected on the `orders` row after some invoices are already filed (a documented pattern in this codebase's history of direct-SQL fixes), old rows stop matching the new prefix and numbering can restart from 1, colliding with files already in Drive under the old naming. Also unverified: whether `orders.order_date` can be NULL — if so, a matched order with a null date would literally embed the string `"None"` in its ship1 filename via `build_filename()`, which has no input validation.

    **Recommendation, not yet decided:** before Agent 01B's recurring Saturday `--run` has been live long enough to accumulate real multi-shipment history, this deserves a proper ADR — one authoritative shipment/invoice count per order_id, written by every producer (Agent 01B, Agent 01D, Agent 01E, and `db_writer.write_invoice()` if it's still live), rather than each agent independently counting a different table/column it happens to write to. Until then, the practical risk is low for the common case (one PDF per email, no hand-edited order numbers) but real for split-shipment orders with multi-attachment emails or any manual Supabase correction.
