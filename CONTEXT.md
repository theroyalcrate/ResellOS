...
23. **Agent 01B's live Gmail-filing pipeline needs a scheduling decision — same shape as the Agent 01E gap just fixed above: it works correctly when run, but nothing makes it run regularly (found 2026-09-19, Cowork session).** The business Gmail account's `ResellOS-Invoices` label currently holds **1,169 messages** (confirmed live, spanning 2025-02-23 through 2026-09-15 — as recent as 2 days before this was checked), all genuine LEGO receipt/order-confirmation emails, and the label is still actively receiving new mail today — the intake side is not broken. But the `invoice_files` ledger shows only **10 rows total** were ever created by the live Gmail-filing pipeline (as opposed to the separate Drive-backfill batch) across the pipeline's entire history, all in two short manual bursts (1 on 2026-07-19, 9 on 2026-08-18) — not a steady trickle, and not "around September 4th" as an earlier note assumed (direct query confirms August 18 as the actual last activity). In other words: Agent 01B's Gmail-intake mode has essentially never been run as an ongoing process — it's been manually invoked at most twice, while well over a thousand messages have piled up untouched since February 2025. **Not fixed, not scheduled** — this needs the same kind of decision Agent 01E just got (a `--run`-style non-interactive mode plus a Windows Task Scheduler job), but Agent 01B hasn't been checked for whether its current interactive-only design would need the same non-interactive treatment first. Investigation only this session, per explicit instruction — no code changed, nothing scheduled.
